...
starting_softmax_neurons = 2
feature_d = 1024
feature_w = 4
feature_h = 4
feature_extractor_size = feature_d*feature_w*feature_h
second_last_layer_size = 1000
class CloudClassifier(nn.Module):
    def __init__(self):
        super(CloudClassifier, self).__init__()
        self.fc1 = nn.Linear(feature_extractor_size,second_last_layer_size)
        self.drop1 = nn.Dropout(0.30)
        self.softmax_layer = nn.Linear(second_last_layer_size,starting_softmax_neurons) #2 output nodes initially
...
number_of_weights_list = []
number_of_useful_update_list = []
total_params_list = []
...
def Cloud(Feature):
    ...
    cloud_learning_rate = 0.0035
    cloud_training_epochs = 3
    ...
    if(task_character == 't'): #train the model
        ...
        cloud_cl.softmax_layer = nn.Linear(second_last_layer_size,cloud_total_nerons_in_final_layer)

        print("cloud_total_nerons_in_final_layer: "+str(cloud_total_nerons_in_final_layer))

        new_weights = -0.0001 * np.random.random_sample((starting_softmax_neurons, second_last_layer_size)) + 0.0001 #RANDOMLY INITIALIZED WEIGHTS FOR THE NEW TASKS
...
#==========MAIN PROGRAM==========
squeezenet = models.shufflenet_v2_x1_0(pretrained=True) #Shufflenet models
squeezenet = torch.nn.Sequential(*(list(squeezenet.children())[:-1])) #remove fc layer from ShuffleNet
...
directory = '/home/swarajdubey611/imagenet_datasets'#r"C:\Users\Swaraj\AppData\Local\Programs\Python\Python36\Journal_paper_codes\SqueezeNet\CUB_200_2011\images"
index_counter = 0
train_counter = 0
test_counter = 0
class_counter = -1
...
total_classes = 6
number_of_increments = int(total_classes/starting_softmax_neurons) #total number of classes to be learnt
img_dimension = 100
...
for subdir in os.listdir(directory):
    if(edge_incremental_i < total_classes):# (classes_per_training_round_count < number_of_classes_to_be_added_ever_increment) and (incremental_i < number_of_increments)
        ...
        for f in os.listdir(subdir_absolute_path):
            images_per_class += 1 #total images in a class
        print("folder: "+str(subdir)+" has number of images: "+str(images_per_class))
        ...
        for f in os.listdir(subdir_absolute_path):
            ...
            if(img.shape[0] == 3):#accept only RGB images
                ...
                training_imgs = 0.80*images_per_class
                if(image_counter < training_imgs): #80% are training images
            ...
            image_counter += 1
            read_toc = time.time()
        print("training_image_list shape: "+str(training_image_list.shape))
        print("testing_image_list shape: "+str(testing_image_list.shape))    
        ...
        if(classes_per_training_round_count == number_of_classes_to_be_added_ever_increment-1): #Send to the cloud for training
            ...
            print("testing_overall_string_tensor length: "+str(len(testing_overall_string_tensor)))
...
print("TOTAL samples_transmitted: "+str(sum(new_images_length_list)))
print("datasize_sent_list: "+str(datasize_sent_list))
