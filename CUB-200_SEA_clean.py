...
def Cloud(Feature):
    ...
    cloud_learning_rate = 0.0015
    cloud_training_epochs = 25
...
#==========MAIN PROGRAM==========
#LOAD TRAIN TEST SPLIT FILE HERE TO KNOW WHICH IMAGE IS IN TRAINING SET AND TESTING SET
train_indexes = [] #this will store indexes that tells at which index image will be in train set
test_indexes = []
file_path = '/home/swarajdubey611/CUB_200_2011/train_test_split.txt'
f = open(file_path, 'r')
lines = f.readlines()
f.close()
train_counter = 0
...
for i in range(len(lines)):
    ...
    if(ind == 1):
        train_indexes.append(train_counter)
    else:
        test_indexes.append(test_counter)
    train_counter += 1
    test_counter += 1
...
directory = '/home/swarajdubey611/CUB_200_2011/Images'#r"C:\Users\Swaraj\AppData\Local\Programs\Python\Python36\Journal_paper_codes\SqueezeNet\CUB_200_2011\images"
index_counter = 0
train_counter = 0
test_counter = 0
class_counter = -1
    ...
    if(edge_incremental_i < total_classes):# (classes_per_training_round_count < number_of_classes_to_be_added_ever_increment) and (incremental_i < number_of_increments)
        ...
        for f in os.listdir(subdir_absolute_path):
            ...
            if(img.shape[0] == 3):#accept only RGB images
                ...
                if(index_counter in train_indexes): #training images
