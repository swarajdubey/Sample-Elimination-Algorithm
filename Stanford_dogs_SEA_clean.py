...
#FUNCTION TO CHECK IF SPARSITY REALLY WORKS OR NOT
def CheckSparsity(cloud_cl):
    useful_weights_count = 0
    total_weights = 0
    for _,layer in cloud_cl.named_modules():
        if(isinstance(layer,nn.Linear)):
            weight_tensor = layer.weight.data #Get weight matrix of the nn.Linear layer
            x = torch.zeros(weight_tensor.shape)
            x[weight_tensor != 0] = 1 #all elements not equal to 0 are assigned as 1 in tensor `x`
            useful_weights_count += torch.count_nonzero(x).item()
            total_weights += torch.numel(x)
    print("useful_weights_count: "+str(useful_weights_count))
    pruned_weights_count = total_weights - useful_weights_count
    pruned_weights_percent = (pruned_weights_count/total_weights)*100.0
    return total_weights, pruned_weights_count, pruned_weights_percent
...
def RandomPruning(cloud_cl,device,Trainloader):
    
    acceptable_training_loss = 0.5
    total_nn_weights, pruned_weights, pruned_weights_percent = CheckSparsity(cloud_cl)
    print("pruned_weights_percent: "+str(pruned_weights_percent))

    #Get baseline accuracy
    baseline_accuracy = GetTrainingAccuracy(cloud_cl,device,Trainloader)

    best_models = []
    #Prune for every layer
    for _,layer in cloud_cl.named_modules():
        if(isinstance(layer,nn.Linear)):
            weight_tensor = copy.deepcopy(layer.weight.data) #Get weight matrix of the nn.Linear layer
            tensor_size = 1
            for i in range(len(weight_tensor.shape)):
                tensor_size *= weight_tensor.shape[i]

            sparsity_increment = 1 #1%
            sparsity_percent = 0.0 #0%
            weights_before_sparsification = 0
            weights_before_sparsification = copy.deepcopy(layer.weight.data) #save the weights of layer l before going sparse
            for i in range(0,100,sparsity_increment):
                sparsity_percent += i #This is the new sparsity level
                sparsity_percent /= 100.0 #convert in range 0 to 1
                print("sparsity_percent is: "+str(sparsity_percent))
                no_of_elements_to_be_sparse = int(sparsity_percent * tensor_size) #number of weights to be pruned
                if(no_of_elements_to_be_sparse > 0): #Sparsity only takes place if the
                    random_indices = random.sample(range(0, tensor_size-1), no_of_elements_to_be_sparse)
                    random_indices = torch.LongTensor(random_indices) #indices to be made sparse
                    
                    weight_tensor = weight_tensor.view(-1,tensor_size) #flatten the weight matrix
                    weight_tensor[0][random_indices] = 0 #prune the parameters
                    layer.weight.data = weight_tensor.view(layer.weight.data.shape)
                best_models.append(cloud_cl)
                temp_accuracy = GetTrainingAccuracy(cloud_cl,device,Trainloader)
            
                print("temp_accuracy: "+str(temp_accuracy))
                if(abs(baseline_accuracy - temp_accuracy) > acceptable_training_loss):
                    if(len(best_models)>=2): #just to ensure no crashing takes place
                        cloud_cl = best_models[-2]
                        break
                else: #model did not prune well enough so restart again
                    layer.weight.data = weights_before_sparsification

    total_nn_weights, pruned_weights, pruned_weights_percent = CheckSparsity(cloud_cl)
    print("pruned_weights_percent: "+str(pruned_weights_percent))

    return cloud_cl, total_nn_weights, pruned_weights, pruned_weights_percent


def MagnitudePruning(cloud_cl,device,Trainloader):
    acceptable_training_loss = 0.5
    total_nn_weights, pruned_weights, pruned_weights_percent = CheckSparsity(cloud_cl)
    print("pruned_weights_percent: "+str(pruned_weights_percent))
    ...
    for _,layer in cloud_cl.named_modules():
        if(isinstance(layer,nn.Linear)):
            ...
            for i in range(len(weight_tensor.shape)):
                tensor_size *= weight_tensor.shape[i]
            flattened_vector = weight_tensor.view(-1,tensor_size)
            sorted_tensors, _ = torch.sort(flattened_vector,descending=False)
            ...
            for i in range(0,100,sparsity_increment):
                ...
                no_of_elements_to_be_sparse = int(sparsity_percent * tensor_size) #number of weights to be pruned
                if(no_of_elements_to_be_sparse > 0): #Sparsity only takes place if the
                    cut_off_weight = sorted_tensors[0][no_of_elements_to_be_sparse-1].item()
                    layer.weight.data = torch.where(weight_tensor.double() > cut_off_weight, weight_tensor.double(), 0.0).float()
                best_models.append(cloud_cl)
                temp_accuracy = GetTrainingAccuracy(cloud_cl,device,Trainloader)
                print("temp_accuracy: "+str(temp_accuracy))
                if(abs(baseline_accuracy - temp_accuracy) > acceptable_training_loss):
...
def GetThresholds(weight_difference_tensor):
    thresholds = []
    quantiles = 20
    tensor_size = 1
    for i in range(len(weight_difference_tensor.shape)):
        tensor_size *= weight_difference_tensor.shape[i]
    flattened_vector = weight_difference_tensor.view(-1,tensor_size)
    sorted_tensors, _ = torch.sort(flattened_vector,descending=False)

    index_of_first_positive_element = (sorted_tensors < 0).sum().item()
    spacing = int(len(sorted_tensors[0])/quantiles)
    for i in range(index_of_first_positive_element,len(sorted_tensors[0]),spacing):
        thresholds.append(sorted_tensors[0][i].item())
    
    return thresholds


def SpecialThresholds(thresholds):
    special_threshold_indices = []
    quarter_index_threshold = thresholds[int(0.25*len(thresholds))]
    half_index_threshold = thresholds[int(0.50*len(thresholds))]
    three_quarter_index_threshold = thresholds[int(0.75*len(thresholds))]
    special_threshold_indices.append(quarter_index_threshold)
    special_threshold_indices.append(half_index_threshold)
    special_threshold_indices.append(three_quarter_index_threshold)
    return special_threshold_indices

def ImprovedJuicer(cloud_cl,pre_training_cl,post_training_cl,Trainloader,device):

    acceptable_training_accuracy_loss = 0.5
    baseline_accuracy = GetTrainingAccuracy(cloud_cl,device,Trainloader)
    print("baseline_accuracy: "+str(baseline_accuracy))
    total_useful_weights = 0
    total_nn_weights = 0
    temp_model = copy.deepcopy(post_training_cl)
    ann_layer_counter = -1
    for (module1,module2) in zip(pre_training_cl.modules(),post_training_cl.modules()):
        if((isinstance(module1,nn.Linear)) and (isinstance(module2,nn.Linear))):
            print("EXTRACTING WEIGHTS FOR LAYER: "+str(ann_layer_counter))
            post_weight, pre_weight = module2.weight.data, module1.weight.data
            weight_tensor_difference = abs(post_weight - pre_weight) #get layer l weight difference
            thresholds = GetThresholds(weight_tensor_difference) #get all thresholds for layer l

            
            layer_l_best_weights_list = []
            special_threshold_indices = SpecialThresholds(thresholds) #get thresholds ar 25%, 50% and 75% positions
            starting_threshold_index = 0
            for i in range(len(special_threshold_indices)):
                threshold = special_threshold_indices[i]
                layer_l_weights = torch.where(weight_tensor_difference.double() > threshold, post_weight.double(), pre_weight.double()).float()
                temp_model_layer_counter = -1
                for _,layer in temp_model.named_modules():
                    if((isinstance(layer,nn.Linear)) and (ann_layer_counter == temp_model_layer_counter)):
                        layer.weight.data = layer_l_weights
                        layer_l_best_weights_list.append(layer_l_weights)
                        break
                    temp_model_layer_counter += 1
                temp_accuracy = GetTrainingAccuracy(temp_model,device,Trainloader) #Calculate the training accuracy on temp_model
                if(abs(baseline_accuracy - temp_accuracy)>=acceptable_training_accuracy_loss):
                    break
                else:
                    starting_threshold = threshold
                    starting_threshold_index = thresholds.index(starting_threshold)
            thresholds = thresholds[starting_threshold_index:] #This is the fine tuned threshold list
            print("length of thresholds now: "+str(len(thresholds))+"\n")
            

            total_nn_weights += torch.numel(weight_tensor_difference)
            layer_l_best_weights_list = []
            for threshold in thresholds:
                print("check for threshold: "+str(threshold))
                layer_l_weights = torch.where(weight_tensor_difference.double() > threshold, post_weight.double(), pre_weight.double()).float()
                ...
                print("temp_accuracy: "+str(temp_accuracy))
                if(abs(baseline_accuracy - temp_accuracy)>=acceptable_training_accuracy_loss):
                    #get the previous temp_model
                    layer_l_best_weights = layer_l_best_weights_list[-2]
                    total_useful_weights += (module2.weight.data == layer_l_best_weights).sum().item()
                    module2.weight.data = layer_l_best_weights

                    #Assign layer_l_best_weights to layer l of temp_model
                    temp_model_layer_counter = -1
                    for _,layer in temp_model.named_modules():
                        if((isinstance(layer,nn.Linear)) and (ann_layer_counter == temp_model_layer_counter)):
                            print("Again temp_model_layer_counter: "+str(temp_model_layer_counter))
                            layer.weight.data = layer_l_best_weights
                            break
                        temp_model_layer_counter += 1
    ...
    pruned_weights = total_nn_weights - total_useful_weights
    pruned_weights_percent = (pruned_weights/total_nn_weights)*100.0
    print("total_useful_weights: "+str(total_useful_weights))
    print("pruned_weights_percent: "+str(pruned_weights_percent))
    return post_training_cl, total_nn_weights, pruned_weights, pruned_weights_percent
...
def Cloud(Feature):
    ...
    if(task_character == 't'): #train the model
        ...
        #juicer algorithm goes here
        pruning_tic = time.time()
        cloud_cl, total_nn_weights, pruned_weights, pruned_weights_percent = ImprovedJuicer(cloud_cl,pre_training_cl,post_training_cl,Trainloader,device)
        
        pruned_weights_percent_list.append(pruned_weights_percent)
        pruned_weights_list.append(pruned_weights)
        total_nn_weights_list.append(total_nn_weights)
        pruning_toc = time.time()
        pruning_time = pruning_toc - pruning_tic
